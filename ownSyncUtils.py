...
class ownClient():
    ...
    def __init__(self, url):
        """
        The URL in http or https format to the owncloud server,
        the remote.php/webdav is required
        """
...
#                                newEntry['etag'] = ETAG.text
                            if lastMod is not None:
                                try:
                                    ...
                                    newEntry['lastMod'] = int(
                                        (time.mktime(T) - time.altzone) * 1000)
                                except Exception as e:
                                    self.log.error(
                                        "Problem converting time stamp: %s, %s"
    ...
    def updateModTime(self, path, ftime):
        ...
        DATA = """
<?xml version='1.0' encoding='UTF-8' ?><D:propertyupdate xmlns:D='DAV:'><D:set>
<D:prop><D:lastmodified>%d</D:lastmodified></D:prop></D:set>
</D:propertyupdate>""" % (ftime)
        r, c = self.http.request(self.url + "/" + urlquote(path),
                                 'PROPPATCH', body=DATA)

    def mkdir(self, path):
        """
        mkdir creates a dirctory on owncloud, it will create the full path
        even if parent directories do not exist
        """
    ...
    def delete(self, path):
        """
        delete deletes any path/file on the owncloud server,
        and will do so recursivly.
        """
    ...
    def addFile(self, newFile, path):
        """
        This adds the given file to the owncloud server.
        newFile is a string path to a local file and that file name
        will be used as its name.
        """
        self.log.debug("Adding New File: %s/%s"
                       % (path, os.path.basename(newFile)))
        ...
        r, c = self.http.request(
            str("{}/{}/{}".format(
                self.url,
                urlquote(path),
                urlquote(os.path.basename(newFile)))),
            "PUT",
            body=data)
            ...
            for f in FILES:
                ...
                if newfile in self.FILES:
                    if FILES[f]['lastMod'] > self.FILES[newfile]['lastMod']:
                        ...
                        self.delete(newfile)
                        self.addFile("%s/%s" % (path, f),
                                     fixPath(os.path.dirname(newfile) + "/"))
                ...
                if f[:len(base)] == base:
                    ...
                    if newfile in FILES:
                        if self.FILES[f]['lastMod'] > FILES[newfile]['lastMod']:
                            ...
                            os.utime("%s/%s" % (path, newfile),
                                     (self.FILES[f]['lastMod'] / 1000,
                                      self.FILES[f]['lastMod'] / 1000))
                    else:
        ...
        if os.path.isdir(path):
            ...
            for d in DIRS:
                ...
                if newpath not in self.DIRS and newpath != "/":
                    try:
                        self.log.debug("Removing local directory %s/%s"
                                       % (path, d))
            ...
            for d in self.DIRS:
                if d[:len(base)] == base:
                    ...
                    if newpath not in DIRS:
                        try:
                            self.log.debug("Creating local directory %s/%s"
                                           % (path, newpath))
                            os.makedirs("%s/%s" % (path, newpath))
                        except Exception as e:
                            pass
            ...
            for f in self.FILES:
                if f[:len(base)] == base:
                    ...
                    if newfile not in FILES:
                        self.log.info(
                            "Creating New file {} {}".format(f, newfile))
...
def fixPath(path):
    ...
    of /'s in them. I could not get any of the os.path fuctions to do this
    reliably, not sure why.
    """
    if path[0] != "/":
        path = "/" + path
    while path.find("//") != -1:
