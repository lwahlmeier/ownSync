...
The options are:
\tto - Local is seen as the master repo, everything remote is replaced,
\t\tupdated or deleted from.
\tfrom - Remote is seen as the master repo, everything local will be replaces,
\t\tupdated or deleted till it looks exactly like whats on the server.
\tboth - Local and Remote paths are compared and merged.
\t\tThe newest file will be used in both places."""
    parser = argparse.ArgumentParser(
        description="Command line tool used to sync your ownCloud files to"
                    " and from local directory",
        formatter_class=RawTextHelpFormatter)
    parser.add_argument('--url',
                        help="url to use to connect to ownCloud "
                             "(IE https://myCloud.com/owncloud/)",
                        required=True)
